...
class AdminReportExport implements FromCollection, WithHeadings, WithMapping, ShouldAutoSize, WithStyles, WithEvents
{
    ...
    {
        $userIds = User::where('user_type', 'client')->pluck('id')->toArray();
        $query = Order::whereIn('client_id', $userIds)
            ->with('payment', 'client', 'city')
            ->where('status', 'completed');

        if ($this->request->filled('client_id')) {
            $query->where('client_id', $this->request->input('client_id'));
        }
    ...
    public function headings($exportType = 'excel'): array
    {
        ...
        if ($exportType === 'excel') {
            $headings[] = [__('message.admin_earning_report') . ($date ? ' : ' . $date : '')];
            $headings[] = [];
            $columnHeadings = [];
    ...
    {
        return getPriceFormat($this->totalAmountSum);
    }
    ...
    {
        return getPriceFormat($this->totalAmountorder);
    }
}