...
class ReportOfUserExport implements FromCollection,WithHeadings, WithMapping, ShouldAutoSize, WithEvents, WithStyles
{
    /**
    ...
    */
    ...
    public function collection()
    {
        ...
        $ordersQuery = Order::query()->with('client')->where('status', 'completed');

        if ($this->request->filled('client_id')) {
            $ordersQuery->where('client_id', $params['client_id']);
        }
    ...
    public function headings($exportType = 'excel'): array
    {
        ...
        $clientName = User::where('user_type', 'client')->where('id', $clientId)->first();
        $clientnameget = $clientName ? $clientName->name : '-';

        if ($exportType === 'excel') {
            $headings[] = [ __('message.report_of') . $clientnameget . ' ' .($date ? ' : ' . $date : '')];
            $headings[] = [];
            $columnHeadings = [];
