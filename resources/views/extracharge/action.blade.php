...
@if($delete_at != null)
    <div class="d-flex justify-content-end align-items-center">
        ...
        @endif
        {{ html()->form('DELETE', route('extracharge.force.delete', [$id, 'type' => 'forcedelete']))->attribute('data--submit', 'extracharge'.$id) }}
 ...
 @else
    ...
    @if($action_type == 'action')
        <div class="d-flex justify-content-end align-items-center">
            ...
            @if($auth_user->can('extracharge-delete'))
                {{ html()->form('DELETE', route('extracharge.destroy', $id))->attribute('data--submit', 'extracharge'.$id)->open() }}
                    <a class="mr-2 text-danger" href="javascript:void(0)" data--submit="extracharge{{$id}}"
                        data--confirmation='true' data-title="{{ __('message.delete_form_title',['form'=> __('message.extracharge') ]) }}"
